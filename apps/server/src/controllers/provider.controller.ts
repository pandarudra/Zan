...
export const registerProvider = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (existing) {
      return res
        .status(409)
        .json({ error: "Machine already registered for this account" });
    }
    ...
    if (!providerWallet) {
      return res
        .status(400)
        .json({
          error: "Provider walletAddress required for staking verification",
        });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({
          error: "walletAddress does not match your linked account wallet",
        });
    }
    ...
    if (!stakeOk) {
      return res
        .status(400)
        .json({ error: "Stake transaction verification failed" });
    }
...
export const connectAgent = async (req: Request, res: Response) => {
  ...
  if (!providerId || !agentPublicKey) {
    return res
      .status(400)
      .json({ error: "providerId and agentPublicKey required" });
  }
...
export const getAvailableProviders = async (req: Request, res: Response) => {
  ...
  try {
    const providers = await prisma.provider.findMany({
      ...
      orderBy: [{ tier: "desc" }, { reputationScore: "desc" }],
      select: {
        id: true,
        gpuModel: true,
        vramGB: true,
