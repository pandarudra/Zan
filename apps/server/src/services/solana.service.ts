...
function loadKeypair(secret: string): Keypair {
  try {
    ...
    throw new Error(
      "Failed to load SOLANA_AUTHORITY_KEYPAIR. Must be base58 or JSON array.",
    );
  }
}
...
export async function verifyStakeTransaction(
  ...
  try {
    ...
    logger.error({ err }, "[SolanaService] verifyStakeTransaction failed");
    return false;
  }
}
...
export async function verifyJobEscrow(
  ...
  try {
    ...
    logger.error({ err }, "[SolanaService] verifyJobEscrow failed");
    return false;
  }
}

export async function assignProviderToEscrow(
  jobIdNum: number,
